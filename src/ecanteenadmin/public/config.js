...
(function () {
    ...
    app.config(['NgAdminConfigurationProvider', 'RestangularProvider', function (NgAdminConfigurationProvider, RestangularProvider) {
        ...
        var admin = nga.application('e食堂后台管理系统') // application main title
            .debug(true) // debug disabled
            .baseApiUrl('http://api.ecanteen.com/api/'); // main API endpoint

        // define all entities at the top to allow references between them
        var user = nga.entity('Users'); // the API endpoint for posts will be http://cpanel.ecanteen.com/:id

        var comment = nga.entity('comments')
            .baseApiUrl('http://api.ecanteen.com/api/') // The base API endpoint can be customized by entity
        ...
        user.listView()
            .title('All customers') // default title is "[Entity_name] list"
            .perPage(10)
            .description('List of customers with infinite pagination') // description appears under the title .infinitePagination(true) // load pages as the user scrolls
            .fields([
                nga.field('id').label('id'), // The default displayed name is the camelCase field name. label() overrides id
                nga.field('userName'), // the default list field type is "string", and displays as a string
                nga.field('nickName'),  // Date field type allows date formatting
                nga.field('password'), // Float type also displays decimal digits
                nga.field('token')
        ...
        user.creationView()
            .fields([
                nga.field('customer_no') // the default edit field type is "string", and displays as a text input
    ...
    app.directive('userLink', ['$location', function ($location) {
        return {
            restrict: 'E',
            scope: { entry: '&' },
            template: '<p class="form-control-static"><a ng-click="displayUser()">View&nbsp;post</a></p>',
            link: function (scope) {
                scope.displayUser = function () {
                    $location.path('/users/show/' + scope.entry().values.user_id);
                };
            }
        };
    }]);
